...
class LFSLvars : public LFunc
{
private:
	ScopeCreep *scopey;

public:
	LFSLvars(std::string n, ScopeCreep *sc) : LFunc(n)
	{
		scopey = sc;
	}
	FundamentalRef run(FundamentalRef head, std::vector<FundamentalRef> args) override
	{
		auto str = scopey->vars();
		return std::make_shared<FundamentalString>(str);
	};
};
...
//	addFunc("sl.trace", std::make_shared<LFlist>("list"));
	addFunc("sl.vars", std::make_shared<LFSLvars>("sl.vars", &scopey));
//	addFunc("sl.funcs", std::make_shared<LFlist>("list"));
